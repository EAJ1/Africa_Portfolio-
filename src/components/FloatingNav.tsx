...
export function FloatingNav({ isDark }: FloatingNavProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="mt-2 sm:mt-3 w-1 sm:w-1.5 h-20 sm:h-24 rounded-full mx-auto overflow-hidden relative"
        style={{
...
function NavButton({ item, Icon, isActive, isHovered, isDark, index, onClick, onHoverStart, onHoverEnd }: NavButtonProps) {
  ...
  return (
    <motion.button
      ...
      className="relative p-1.5 sm:p-2 rounded-xl transition-all duration-300 group overflow-visible"
    ...
    >
      ...
      {/* Enhanced 3D tooltip */}
      <motion.div
        className="absolute left-full top-1/2 -translate-y-1/2 ml-2 sm:ml-4 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-[100000]"
        style={{
          background: `linear-gradient(135deg, ${item.color}, ${item.color}dd)`,
